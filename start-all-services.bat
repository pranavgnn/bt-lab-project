...
start "Customer Service" cmd /k "cd customer && mvn spring-boot:run"

timeout /t 5 /nobreak > nul

echo Starting Product Service on port 8082...
start "Product Service" cmd /k "cd product-pricing && mvn spring-boot:run"

timeout /t 5 /nobreak > nul

echo Starting FD Calculator Service on port 8083...
start "FD Calculator Service" cmd /k "cd fd-calculator && mvn spring-boot:run"

timeout /t 5 /nobreak > nul

echo Starting Accounts Service on port 8084...
start "Accounts Service" cmd /k "cd accounts && mvn spring-boot:run"

timeout /t 10 /nobreak > nul

echo Starting Main Gateway with Frontend on port 8080...
start "Main Gateway" cmd /k "cd main && mvn spring-boot:run"
